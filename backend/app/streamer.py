...
async def db_writer(queue):
    ...
    batch_size = 1000 # Увеличили размер пачки для снижения нагрузки (было 30)
    buffer = []
    ...
    while True:
        try:
            ...
            # Если буфер не полный, даем шанс накопиться (1 сек)
            # Это снижает IOPS базы данных, но добавляет задержку 1с (приемлемо)
